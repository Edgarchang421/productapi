...
class ProductDetailTest(TestCase): #測試ProductDetail的GET、PUT和DELET，分為匿名user和已驗證過的user。
	...
	def test_AuthenticatedUser_but_not_owner_get(self): #測試已驗證的user，但並非owner使用get method
	...
	def test_AuthenticatedUser_but_not_owner_put(self): #測試已驗證的user，但並非owner使用put method
		JWT = self.get_not_owner_JSON_Web_Token()
	...
	def test_AuthenticatedUser_but_not_owner_delete(self): #測試已驗證的user，但並非owner使用delete method
		JWT = self.get_not_owner_JSON_Web_Token()
